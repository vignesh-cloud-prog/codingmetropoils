import { BsPersonGear } from "react-icons/bs"
import { TfiRulerPencil } from "react-icons/tfi"
import { BiUser } from "react-icons/bi"
import { MdOutlineVerifiedUser } from "react-icons/md"

export const home = [
  {
    icon: <BsPersonGear size={25} />,
    title: "Built for local SMBs and startups",
  },
  {
    icon: <TfiRulerPencil size={25} />,
    title: "Software, AI, and marketing in one stack",
  },
  {
    icon: <MdOutlineVerifiedUser size={25} />,
    title: "Clear plans — Launch, Growth, and Scale",
  },
  {
    icon: <BiUser size={25} />,
    title: "Custom enterprise tools when you outgrow packages",
  },
]
export const expertise = [
  {
    id: 1,
    title: "Websites & CRM",
    cover: "/images/e1.jpg",
    desc: [{ text: "Marketing sites" }, { text: "Landing pages" }, { text: "Website + CRM" }, { text: "Lead capture" }, { text: "Booking flows" }],
  },
  {
    id: 2,
    title: "AI Growth Stack",
    cover: "/images/e2.jpg",
    desc: [{ text: "AI social media" }, { text: "AI sales process" }, { text: "Support agents" }, { text: "Custom workflows" }],
  },
  {
    id: 3,
    title: "Apps & MVP",
    cover: "/images/e3.jpg",
    desc: [{ text: "Startup MVP" }, { text: "Web apps" }, { text: "Cross-platform apps" }, { text: "Admin dashboards" }],
  },
  {
    id: 4,
    title: "Marketing & Enterprise",
    cover: "/images/e4.jpg",
    desc: [{ text: "SEO & content" }, { text: "Paid ads" }, { text: "Custom AI tools" }, { text: "Integrations" }],
  },
]
export const testimonial = [
  {
    id: 1,
    name: "Anjali Spices",
    cover: "/images/e1.jpg",
    post: "Founder · Anjali Spices",
    offer: "Launch",
    built: "Landing page",
    desc: "Our new landing page gave us a clear digital identity and a professional first impression for customers discovering us online.",
    url: "https://spices-landing-e-commerce.vercel.app/",
  },
  {
    id: 2,
    name: "Karunya Special School",
    cover: "/images/e2.jpg",
    post: "Head Master · Malpe",
    offer: "Growth",
    built: "Website + admin dashboard",
    desc: "The digital gallery and editable admin dashboard helped us showcase our work with authority — and made it easier to attract more donations.",
    url: "https://www.karunyaspecialschool.in/",
  },
  {
    id: 3,
    name: "Hakuna Matata Team",
    cover: "/images/e3.jpg",
    post: "Product Team",
    offer: "Scale",
    built: "Customer reviews app",
    desc: "The customer review app is clean, easy to use, and actually helps us collect feedback and stay in touch with customers.",
    url: "/appointment?intent=software",
  },
  {
    id: 4,
    name: "FocusStreak",
    cover: "/images/e4.jpg",
    post: "Founders",
    offer: "MVP",
    built: "Startup app",
    desc: "CodeMadeBiz helped us launch our first app with earning features and a product experience users are happy to review.",
    url: "https://focusstreak.codemadebiz.com/",
  },
  {
    id: 5,
    name: "Paidr",
    cover: "/images/s3.jpg",
    post: "Product Team",
    offer: "Scale",
    built: "Fintech product",
    desc: "We needed a polished product experience for payments workflows — the build was focused, fast, and ready for real users.",
    url: "https://paidr.codemadebiz.com/",
  },
]
export const showcase = [
  {
    id: 1,
    title: "Neon Lights",
    cover: "/images/s1.jpg",
    catgeory: "DIGITAL MARKETING",
  },
  {
    id: 2,
    title: "Futuristic Furniture",
    catgeory: "WEBSITES",
    cover: "/images/s2.jpg",
  },
  {
    id: 3,
    title: "Smart Living",
    cover: "/images/s3.jpg",
    catgeory: "WEBSITES",
  },
  {
    id: 4,
    title: "Light Painting",
    cover: "/images/s4.jpg",
    catgeory: "BRANDING",
  },
  {
    id: 5,
    title: "Ideabox",
    cover: "/images/s5.jpg",
    catgeory: "BRANDING",
  },
  {
    id: 6,
    title: "VR Experience",
    cover: "/images/s6.jpg",
    catgeory: "WEBSITES",
  },
]
export const brand = [
  {
    id: 1,
    cover: "/images/l1.svg",
  },
  {
    id: 2,
    cover: "/images/l2.svg",
  },
  {
    id: 3,
    cover: "/images/l3.svg",
  },
  {
    id: 4,
    cover: "/images/l4.svg",
  },
  {
    id: 5,
    cover: "/images/l5.svg",
  },
  {
    id: 6,
    cover: "/images/l6.svg",
  },
]
export { latestArticles2026 as blogdata } from "./latestArticles2026"
export const teamdata = [
  {
    id: 1,
    title: "Alexander Black",
    cover: "/images/t1.jpg",
    post: "FOUNDER, CEO",
  },
  {
    id: 2,
    title: "Anna Kovalenko",
    cover: "/images/t2.jpg",
    post: "FINANCE DIRECTOR",
  },
  {
    id: 3,
    title: "Tiffany White",
    cover: "/images/t3.jpg",
    post: "CREATIVE DIRECTOR",
  },
  {
    id: 4,
    title: "Richard Greenwood",
    cover: "/images/t4.jpg",
    post: "PROGRAMMER",
  },
  {
    id: 5,
    title: "Jessica Brown",
    cover: "/images/t5.jpg",
    post: "MARKETING DIRECTOR",
  },
  {
    id: 6,
    title: "Gregory Windstorm",
    cover: "/images/t6.jpg",
    post: "ACCOUNTING MANAGER",
  },
  {
    id: 7,
    title: "Anna Red",
    cover: "/images/t7.jpg",
    post: "PROJECT MANAGER",
  },
  {
    id: 8,
    title: "Join our team!",
    cover: "/images/t8.jpg",
    post: "",
  },
]
